...
int main(int argc, char **argv) {
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "hf:l:e:b:i:s:", long_options, &option_index);
		...
		case 'b':
			try {
				batch_size = stoi(optarg);
			} catch (...) {
				cout << "batch_size must be integer";
				exit(1);
			}
			break;
	...
	// try {
		...
		LSTM_training_res res = nn.train(data, epochs, batch_size, learning_rate);
...
void print_help(string binary_path) {
	...
	cout << "	-b --batch-size\t\tNumber of samples in batch" << endl;
